...
namespace SysWork.Data.DaoModel
{
    ...
    public abstract class BaseDao<T>: IBaseDao<T> where T : class, new() 
    {
        ...
        /// </summary>
        /// <param name="ConnectionString">Representa la cadena de conexion</param>
        /// <param name="ResolverDbType">El motor de base de datos a utilizar</param>
        public BaseDao(string ConnectionString, EDataBaseEngine ResolverDbType)
        {
            BaseDaoConstructorResolver(ConnectionString, ResolverDbType);
        }

        private void BaseDaoConstructorResolver(string ConnectionString, EDataBaseEngine resolverDataBaseEngine)
        {
            _dataBaseEngine = resolverDataBaseEngine;
        ...
        public long Add(T entity)
        {
            ...
            if (parameterList.ToString() != string.Empty)
            {
                ...
                insertQuery.Append(string.Format("INSERT INTO [{0}] ( {1} ) VALUES ( {2} ) {3}  ", TableName, ColumnsForInsert, parameterList, GetSubQueryScalar()));
                try
                {
        ...
        private string GetSubQueryScalar()
        {
        ...
        private ColumnDbInfo GetColumnDbInfo(string columnName)
        {
            ...
            using (DbConnection dbConnection = GetDbConnection())
            {
                ...
                DataTable columnProperty = dbConnection.GetSchema("Columns", new[] { dbConnection.Database, null, TableName,columnName});
                DataRow dataRow  = columnProperty.Rows[0];

                columnData.MaxLenght = 0;
