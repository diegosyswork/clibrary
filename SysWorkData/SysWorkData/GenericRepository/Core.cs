...
namespace SysWork.Data.GenericRepostory
{
    ...
    public abstract class BaseGenericRepository<TEntity>: IBaseGenericRepository<TEntity> where TEntity : class, new() 
    {
        ...
        ///     }
        ///     
        ///     public Person GetByFirsNameLastName(string firstName, string lastName)
        ///     {
        ///         return GetByLambdaExpressionFilter(entity => (entity.FirsName == firsName && entity.LastName == lastName));
        ///     }
        ///}
        ///
        ///class Test
        ...
        ///                 
        ///                 if (result )
        ///                     Console.WriteLine($"The passport of {person.FirsName} was updated correctly. {recordsAffected} was affected");
        ///                 else
        ///                     Console.WriteLine($"Error updating the passport of {person.FirsName}, Error: {errMessage} );
        ///                 
        ///             }
        ///         }    
        ...
        ///                 
        ///                 if (result )
        ///                     Console.WriteLine($"The passport of {person.FirsName} was updated correctly.");
        ///             }
        ///         }    
        ///         catch(ArgumentException ae)
        ///         {
        ...
        public IList<TEntity> GetAll(IDbConnection paramDbConnection,IDbTransaction paramDbTransaction)
        {
            ...
            try
            {
                ...
                IDataReader reader = dbCommand.ExecuteReader();
                collection = new IDataReaderToEntity().Map<TEntity>(reader, ListObjectPropertyInfo,_dataBaseEngine);
        ...
        public IList<TEntity> GetListByLambdaExpressionFilter(Expression<Func<TEntity, bool>> lambdaExpressionFilter, IDbConnection paramDbConnection,IDbTransaction paramDbTransaction)
        {
            ...
            try
            {
                ...
                IDataReader reader = dbCommand.ExecuteReader();
                resultado = new IDataReaderToEntity().Map<TEntity>(reader, ListObjectPropertyInfo,_dataBaseEngine);
        ...
        public IList<TEntity> Find(IEnumerable<object> ids, IDbConnection paramDbConnection, IDbTransaction paramDbTransaction)
        {
            ...
            if (clause.ToString() != string.Empty)
            {
                ...
                try
                {
                    ...
                    IDataReader reader = dbCommand.ExecuteReader();
                    entities = new IDataReaderToEntity().Map<TEntity>(reader, ListObjectPropertyInfo,_dataBaseEngine);
        ...
        private ColumnDbInfo GetColumnDbInfo(string columnName,DbConnection dbConnection)
        {
            ...
            if (_dataBaseEngine == EDataBaseEngine.OleDb)
            {
                if (s_sqlTypeToDbType.TryGetValue(((OleDbType)dataType).ToString(), out DbType dBTypeValue))
                    ...
                    throw new ArgumentOutOfRangeException("The datatype is not recognited. See the s_sqlTypeToDbType Dictionary");
            }
            else
            {
        ...
        /// <summary>
        /// Creates the an database parameter.
        /// </summary>
    ...
    public class IDataReaderToEntity
    {
            ...
            while (reader.Read())
            {
                ...
                foreach (PropertyInfo i in _propertyInfo)
                {
                    try
                    {
                        ...
                        var columnName = _syntaxProvider.GetSecureColumnName(custumAttribute.ColumnName ?? i.Name);

                        if ((custumAttribute).Convert == true)
                        {
        ...
        public TEntity MapSingle<TEntity>(IDataRecord dataRecord, IList<PropertyInfo> listObjectPropertyInfo) where TEntity : class, new()
        {
            ...
            foreach (PropertyInfo i in _propertyInfo)
            {
                ...
                {
                    var custumAttribute = (DbColumnAttribute)i.GetCustomAttribute(typeof(DbColumnAttribute));
                    var columName =  _syntaxProvider.GetSecureColumnName(custumAttribute.ColumnName ?? i.Name);
